	...
	*/
	...
	struct TrackStruct
	{
		u8  flags;      // b0-b2: reserved
		                ...
		                // b6: hold envelope: 1=hold volume envelope, i.e: don't increase/decrease, 0=don't hold
		                // b7: priority: 1=Hi/Fx, 0=low/regular note

		u8  remTicks;      // Remaining ticks from current patch command
		u8  const *patPtr; // Current Patch position pointer

		u8  note;          // Current playing note

		u8  trackVol;      // 0x00 - 0xFF => 0x01 - 0x100 effective
		u8  noteVol;
		u8  envelopeVol;

		s8  envelopeStep;  // Amount of envelope change each frame -128 - +127

		u8  tremoloWave;   // Tremolo parameters
		u8  tremoloPos;
		u8  tremoloLevel;
		u8  tremoloRate;
	};
